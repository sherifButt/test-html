const ads_ = [
    {
        "timestamp": 0,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Podcast Starts Now",
        "text": ["Welcome back to Marketing Insights, the podcast exploring trends, challenges, and opportunities in marketing and business growth. I'm Mike Sanshase, your host."],
        "tags": "#MarketingInsights #PodcastIntro",
        "img": "/img/podcast_intro.webp",
        "imgDescription": "A 3D sketch style drawing of a microphone viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The microphone should appear modern and abstract, set against a strictly white background.",
        "cta": "Stay Tuned",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "hidden-element",
            "text": "slide-from-left",
            "headline": "fade-in",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 10000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Special Guest",
        "text": ["Today, we have Sherif Butt, a seasoned expert with over 10 years of experience, joining us to discuss why businesses hesitate to implement lead generation. Welcome, Sherif!"],
        "tags": "#SpecialGuest #LeadGeneration",
        "img": "/img/sherif.webp",
        "imgDescription": "A 3D sketch style drawing of a business professional viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The professional should appear modern and abstract, set against a strictly white background.",
        "cta": "Listen Now",
        "animation": {
            "image": "slide-from-left",
            "cta": "fade-in",
            "tags": "slide-from-bottom",
            "text": "slide-from-right",
            "headline": "fade-in",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 20000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Hesitation Reasons",
        "text": ["That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective."],
        "tags": "#LeadGeneration #Misconceptions",
        "img": "/img/podcast_intro.webp",
        "imgDescription": "A 3D sketch style drawing of a puzzled business person with question marks above their head, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The person should appear modern and abstract, set against a strictly white background.",
        "cta": "Understand More",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "fade-in",
            "text": "slide-from-left",
            "headline": "slide-from-top",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 30000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Overcoming Misconceptions",
        "text": ["While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming. With the right approach, lead generation can be manageable and highly effective."],
        "tags": "#LeadGeneration #Strategy",
        "img": "/img/lead_generation_strategy.webp",
        "imgDescription": "A 3D sketch style drawing of a strategy plan on a whiteboard viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The whiteboard should appear modern and abstract, set against a strictly white background.",
        "cta": "Learn More",
        "animation": {
            "image": "slide-from-right",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "fade-in",
            "headline": "fade-in",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 40000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Immediate Results",
        "text": ["It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly."],
        "tags": "#LeadGeneration #Results",
        "img": "/img/lead_generation_results.webp",
        "imgDescription": "A 3D sketch style drawing of a business graph showing upward growth, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
        "cta": "See Results",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-top",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 50000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Understanding the Audience",
        "text": ["One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them."],
        "tags": "#LeadGeneration #TargetAudience",
        "img": "/img/understanding_audience.webp",
        "imgDescription": "A 3D sketch style drawing of a target board with an arrow hitting the center, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The target board should appear modern and abstract, set against a strictly white background.",
        "cta": "Know More",
        "animation": {
            "image": "slide-from-right",
            "cta": "slide-from-bottom",
            "tags": "fade-in",
            "text": "slide-from-left",
            "headline": "fade-in",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 60000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Fear of the Unknown",
        "text": ["Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support."],
        "tags": "#LeadGeneration #Expertise",
        "img": "/img/fear_unknown.webp",
        "imgDescription": "A 3D sketch style drawing of a business person standing at a crossroads with multiple signposts, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The crossroads should appear modern and abstract, set against a strictly white background.",
        "cta": "Get Help",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "fade-in",
            "headline": "slide-from-top",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 70000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Practical Solutions",
        "text": ["First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices."],
        "tags": "#LeadGeneration #Education",
        "img": "/img/practical_solutions.webp",
        "imgDescription": "A 3D sketch style drawing of a person reading a book on marketing, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The book should appear modern and abstract, set against a strictly white background.",
        "cta": "Educate Yourself",
        "animation": {
            "image": "slide-from-right",
            "cta": "slide-from-bottom",
            "tags": "fade-in",
            "text": "slide-from-left",
            "headline": "fade-in",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 80000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Start Small",
        "text": ["Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."],
        "tags": "#LeadGeneration #StartSmall",
        "img": "/img/start_small.webp",
        "imgDescription": "A 3D sketch style drawing of a small plant growing from the ground, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The plant should appear modern and abstract, set against a strictly white background.",
        "cta": "Begin Now",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "fade-in",
            "headline": "slide-from-top",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 90000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Leverage Technology",
        "text": ["There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."],
        "tags": "#LeadGeneration #Technology",
        "img": "/img/technology_tools.webp",
        "imgDescription": "A 3D sketch style drawing of a computer screen with various marketing tools displayed, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The screen should appear modern and abstract, set against a strictly white background.",
        "cta": "Use Tools",
        "animation": {
            "image": "slide-from-left",
            "cta": "slide-from-bottom",
            "tags": "fade-in",
            "text": "slide-from-left",
            "headline": "fade-in",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 100000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Partner with Experts",
        "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference."],
        "tags": "#LeadGeneration #Experts",
        "img": "/img/partner_experts.webp",
        "imgDescription": "A 3D sketch style drawing of two business professionals shaking hands, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The handshake should appear modern and abstract, set against a strictly white background.",
        "cta": "Find Experts",
        "animation": {
            "image": "slide-from-right",
            "cta": "slide-from-bottom",
            "tags": "fade-in",
            "text": "slide-from-left",
            "headline": "fade-in",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 110000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Conclusion",
        "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk."],
        "tags": "#LeadGeneration #Conclusion",
        "img": "/img/conclusion.webp",
        "imgDescription": "A 3D sketch style drawing of a closed book with a bookmark, viewed from a slightly elevated angle. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The book should appear modern and abstract, set against a strictly white background.",
        "cta": "Visit Now",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "fade-in",
            "headline": "slide-from-top",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    }
]


const animationTemplates_ = [
    {
        "id": "fade-in",
        "name": "fade-in",
        "entry": {
            "image": "fade-in",
            "cta": "fade-in",
            "tags": "fade-in",
            "text": "fade-in",
            "headline": "fade-in",
            "logoIcon": "fade-in",
            "pathTwo": "fade-in",
            "pathOne": "fade-in"
        },
        "exit": {
            "image": "fade-out",
            "cta": "fade-out",
            "tags": "fade-out",
            "text": "fade-out",
            "headline": "fade-out",
            "logoIcon": "fade-out",
            "pathTwo": "fade-out",
            "pathOne": "fade-out"
        }
    },
    {
        "id": "slide-from-left",
        "name": "slide-from-left",
        "entry": {
            "image": "slide-from-left",
            "cta": "slide-from-left",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "slide-from-left",
            "pathOne": "slide-from-left"
        },
        "exit": {
            "image": "slide-to-left",
            "cta": "slide-to-left",
            "tags": "slide-to-left",
            "text": "slide-to-left",
            "headline": "slide-to-left",
            "logoIcon": "slide-to-left",
            "pathTwo": "slide-to-left",
            "pathOne": "slide-to-left"
        }
    },
    {
        "id": "slide-from-right",
        "name": "slide-from-right",
        "entry": {
            "image": "slide-from-right",
            "cta": "slide-from-right",
            "tags": "slide-from-right",
            "text": "slide-from-right",
            "headline": "slide-from-right",
            "logoIcon": "slide-from-right",
            "pathTwo": "slide-from-right",
            "pathOne": "slide-from-right"
        },
        "exit": {
            "image": "slide-to-right",
            "cta": "slide-to-right",
            "tags": "slide-to-right",
            "text": "slide-to-right",
            "headline": "slide-to-right",
            "logoIcon": "slide-to-right",
            "pathTwo": "slide-to-right",
            "pathOne": "slide-to-right"
        }
    },
    {
        "id": "slide-from-top",
        "name": "slide-from-top",
        "entry": {
            "image": "slide-from-top",
            "cta": "slide-from-top",
            "tags": "slide-from-top",
            "text": "slide-from-top",
            "headline": "slide-from-top",
            "logoIcon": "slide-from-top",
            "pathTwo": "slide-from-top",
            "pathOne": "slide-from-top"
        },
        "exit": {
            "image": "slide-to-top",
            "cta": "slide-to-top",
            "tags": "slide-to-top",
            "text": "slide-to-top",
            "headline": "slide-to-top",
            "logoIcon": "slide-to-top",
            "pathTwo": "slide-to-top",
            "pathOne": "slide-to-top"
        }
    },
    {
        "id": "slide-from-bottom",
        "name": "slide-from-bottom",
        "entry": {
            "image": "slide-from-bottom",
            "cta": "slide-from-bottom",
            "tags": "slide-from-bottom",
            "text": "slide-from-bottom",
            "headline": "slide-from-bottom",
            "logoIcon": "slide-from-bottom",
            "pathTwo": "slide-from-bottom",
            "pathOne": "slide-from-bottom"
        },
        "exit": {
            "image": "slide-to-bottom",
            "cta": "slide-to-bottom",
            "tags": "slide-to-bottom",
            "text": "slide-to-bottom",
            "headline": "slide-to-bottom",
            "logoIcon": "slide-to-bottom",
            "pathTwo": "slide-to-bottom",
            "pathOne": "slide-to-bottom"
        }
    },
    {
        "id": "hidden-element",
        "name": "hidden-element",
        "entry": {
            "image": "hidden-element",
            "cta": "hidden-element",
            "tags": "hidden-element",
            "text": "hidden-element",
            "headline": "hidden-element",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        },
        "exit": {
            "image": "hidden-element",
            "cta": "hidden-element",
            "tags": "hidden-element",
            "text": "hidden-element",
            "headline": "hidden-element",
            "logoIcon": "hidden-element",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "id": "cross-slide",
        "name": "cross-slide",
        "entry": {
            "image": "slide-from-left",
            "cta": "slide-from-right",
            "tags": "slide-from-right",
            "text": "slide-from-right",
            "headline": "slide-from-right",
            "logoIcon": "slide-from-left",
            "pathTwo": "slide-from-bottom",
            "pathOne": "slide-from-bottom"
        },
        "exit": {
            "image": "slide-to-left",
            "cta": "slide-to-right",
            "tags": "slide-to-right",
            "text": "slide-to-right",
            "headline": "slide-to-right",
            "logoIcon": "slide-to-left",
            "pathTwo": "slide-to-bottom",
            "pathOne": "slide-to-bottom"
        }
    }
]

let animationTemplates = [
    {
        "id": "none",
        "name": "none",
        "entry": {
            "settings": {
                "duration": 0,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "fade-in",
                "cta": "fade-in",
                "tags": "fade-in",
                "text": "fade-in",
                "headline": "fade-in",
                "logoIcon": "fade-in",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "fade-in",
                "textContainer": "fade-in",
                "backdrop": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "fade-in"
            }
        },
        "exit": {
            "settings": {
                "duration": 0,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "fade-out",
                "cta": "fade-out",
                "tags": "fade-out",
                "text": "fade-out",
                "headline": "fade-out",
                "logoIcon": "fade-out",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "fade-out",
                "textContainer": "fade-out",
                "backdrop": "fade-out",
                "frontdrop": "fade-out",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "dramatic-entry",
        "name": "dramatic-entry",
        "entry": {
            "settings": {
                "duration": 4000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "zoom-in-sm-to-twist",
                "cta": "fade-in",
                "tags": "fade-in",
                "text": "fade-in",
                "headline": "fade-in",
                "logoIcon": "fade-in",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "zoom-in-sm-to-twist",
                "textContainer": "zoom-in-md",
                "backdrop": "zoom-out-lg-to-stay",
                "filter": "zoom-in-md",
                "frontdrop": "zoom-out-md-to-stay",
                "backbanner": "fade-in"
            }
        },
        "exit": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "zoom-out-sm-to-twist",
                "cta": "fade-out",
                "tags": "fade-out",
                "text": "fade-out",
                "headline": "fade-out",
                "logoIcon": "fade-out",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "fade-out",
                "textContainer": "zoom-out-sm",
                "backdrop": "zoom-in-lg-to-go",
                "filter": "zoom-out-md",
                "frontdrop": "zoom-in-md-to-go",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "fade-in",
        "name": "fade-in",
        "entry": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "fade-in",
                "cta": "fade-in",
                "tags": "fade-in",
                "text": "fade-in",
                "headline": "fade-in",
                "logoIcon": "fade-in",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "fade-in",
                "textContainer": "fade-in",
                "backdrop": "fade-in",
                "filter": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "fade-in"
            }
        },
        "exit": {
            "settings": {
                "duration": 1000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "fade-out",
                "cta": "fade-out",
                "tags": "fade-out",
                "text": "fade-out",
                "headline": "fade-out",
                "logoIcon": "fade-out",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "fade-out",
                "textContainer": "fade-out",
                "backdrop": "fade-out",
                "filter": "fade-out",
                "frontdrop": "fade-out",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "only-text",
        "name": "only-text",
        "entry": {
            "settings": {
                "duration": 1000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "none",
                "cta": "none",
                "tags": "none",
                "text": "bounce-from-left",
                "headline": "bounce-from-left",
                "logoIcon": "none",
                "pathTwo": "none",
                "pathOne": "none",
                "logo-title": "none",
                "textContainer": "none",
                "backdrop": "none",
                "filter": "none",
                "frontdrop": "none",
                "backbanner": "none"
            }
        },
        "exit": {
            "settings": {
                "duration": 1000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "none",
                "cta": "none",
                "tags": "none",
                "text": "bounce-to-left",
                "headline": "bounce-to-left",
                "logoIcon": "none",
                "pathTwo": "none",
                "pathOne": "none",
                "logo-title": "none",
                "textContainer": "none",
                "backdrop": "none",
                "filter": "none",
                "frontdrop": "none",
                "backbanner": "none"
            }
        }
    },
    {
        "id": "slide-from-left",
        "name": "slide-from-left",
        "entry": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-left",
                "cta": "slide-from-left",
                "tags": "slide-from-left",
                "text": "slide-from-left",
                "headline": "slide-from-left",
                "logoIcon": "slide-from-left",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-left",
                "textContainer": "slide-from-left",
                "backdrop": "fade-in",
                "filter": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "fade-in"

            }
        },
        "exit": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-left",
                "cta": "slide-to-left",
                "tags": "slide-to-left",
                "text": "slide-to-left",
                "headline": "slide-to-left",
                "logoIcon": "slide-to-left",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-left",
                "textContainer": "slide-to-left",
                "backdrop": "fade-out",
                "filter": "fade-out",
                "frontdrop": "fade-out",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "slide-from-left-to-right",
        "name": "slide-from-left",
        "entry": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-left",
                "cta": "slide-from-left",
                "tags": "slide-from-left",
                "text": "slide-from-left",
                "headline": "slide-from-left",
                "logoIcon": "slide-from-left",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-left",
                "textContainer": "slide-from-left",
                "backdrop": "fade-in",
                "filter": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "fade-in"
            }
        },
        "exit": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-right",
                "cta": "slide-to-right",
                "tags": "slide-to-right",
                "text": "slide-to-right",
                "headline": "slide-to-right",
                "logoIcon": "slide-to-right",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-right",
                "textContainer": "slide-to-right",
                "backdrop": "fade-out",
                "filter": "fade-out",
                "frontdrop": "fade-out",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "slide-from-left-some-to-right",
        "name": "slide-from-left",
        "entry": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-left",
                "cta": "slide-from-left",
                "tags": "slide-from-left",
                "text": "slide-from-left",
                "headline": "slide-from-left",
                "logoIcon": "slide-from-left",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-left",
                "textContainer": "slide-from-left",
                "backdrop": "fade-in",
                "filter": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "fade-in"
            }
        },
        "exit": {
            "settings": {
                "duration": 2000,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-left",
                "cta": "slide-to-right",
                "tags": "slide-to-right",
                "text": "slide-to-right",
                "headline": "slide-to-right",
                "logoIcon": "slide-to-left",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-left",
                "textContainer": "slide-to-right",
                "backdrop": "fade-out",
                "backbanner": "fade-out"
            }
        }
    },
    {
        "id": "slide-from-right",
        "name": "slide-from-right",
        "entry": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-right",
                "cta": "slide-from-right",
                "tags": "slide-from-right",
                "text": "slide-from-right",
                "headline": "slide-from-right",
                "logoIcon": "slide-from-right",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-right",
                "textContainer": "slide-from-right",
                "backdrop": "fade-in",
                "filter": "fade-in",
                "frontdrop": "fade-in",
                "backbanner": "slide-from-right"
            }
        },
        "exit": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-right",
                "cta": "slide-to-right",
                "tags": "slide-to-right",
                "text": "slide-to-right",
                "headline": "slide-to-right",
                "logoIcon": "slide-to-right",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-right",
                "textContainer": "slide-to-right",
                "backdrop": "fade-out",
                "backbanner": "slide-to-right"
            }
        }
    },
    {
        "id": "slide-from-top",
        "name": "slide-from-top",
        "entry": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-top",
                "cta": "slide-from-top",
                "tags": "slide-from-top",
                "text": "slide-from-top",
                "headline": "slide-from-top",
                "logoIcon": "slide-from-top",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-top",
                "textContainer": "slide-from-top",
                "backdrop": "fade-in",
                "backbanner": "slide-from-top"
            }
        },
        "exit": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-top",
                "cta": "slide-to-top",
                "tags": "slide-to-top",
                "text": "slide-to-top",
                "headline": "slide-to-top",
                "logoIcon": "slide-to-top",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-top",
                "textContainer": "slide-to-top",
                "backdrop": "fade-out",
                "backbanner": "slide-to-top"
            }
        }
    },
    {
        "id": "slide-from-bottom",
        "name": "slide-from-bottom",
        "entry": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-bottom",
                "cta": "slide-from-bottom",
                "tags": "slide-from-bottom",
                "text": "slide-from-bottom",
                "headline": "slide-from-bottom",
                "logoIcon": "slide-from-bottom",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-bottom",
                "textContainer": "slide-from-bottom",
                "backdrop": "fade-in",
                "backbanner": "slide-from-bottom"
            }
        },
        "exit": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-bottom",
                "cta": "slide-to-bottom",
                "tags": "slide-to-bottom",
                "text": "slide-to-bottom",
                "headline": "slide-to-bottom",
                "logoIcon": "slide-to-bottom",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-bottom",
                "textContainer": "slide-to-bottom",
                "backdrop": "fade-out",
                "backbanner": "slide-to-bottom"
            }
        }
    },
    {
        "id": "hidden-element",
        "name": "hidden-element",
        "entry": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "hidden-element",
                "cta": "hidden-element",
                "tags": "hidden-element",
                "text": "hidden-element",
                "headline": "hidden-element",
                "logoIcon": "hidden-element",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "hidden-element",
                "textContainer": "hidden-element",
                "backdrop": "fade-in",
                "backbanner": "hidden-element"
            }
        },
        "exit": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "hidden-element",
                "cta": "hidden-element",
                "tags": "hidden-element",
                "text": "hidden-element",
                "headline": "hidden-element",
                "logoIcon": "hidden-element",
                "pathTwo": "hidden-element",
                "pathOne": "hidden-element",
                "logo-title": "hidden-element",
                "textContainer": "hidden-element",
                "backdrop": "fade-out",
                "backbanner": "hidden-element"
            }
        }
    },
    {
        "id": "cross-slide",
        "name": "cross-slide",
        "entry": {
            "settings": {
                "duration": 500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-from-left",
                "cta": "slide-from-right",
                "tags": "slide-from-right",
                "text": "slide-from-right",
                "headline": "slide-from-right",
                "logoIcon": "slide-from-left",
                "pathTwo": "slide-from-bottom",
                "pathOne": "slide-from-bottom",
                "logo-title": "slide-from-left",
                "textContainer": "slide-from-right",
                "backdrop": "fade-in",
                "backbanner": "slide-from-right"
            }
        },
        "exit": {
            "settings": {
                "duration": 1500,
                "delay": 0,
                "easing": "ease-in-out"
            },
            "elements": {
                "image": "slide-to-left",
                "cta": "slide-to-right",
                "tags": "slide-to-right",
                "text": "slide-to-right",
                "headline": "slide-to-right",
                "logoIcon": "slide-to-left",
                "pathTwo": "slide-to-bottom",
                "pathOne": "slide-to-bottom",
                "logo-title": "slide-to-left",
                "textContainer": "slide-to-right",
                "backdrop": "fade-out",
                "backbanner": "slide-to-right"

            }
        }
    }
]

let settingsTemplates = {
    "default": {
        "id": "default",
        "name": "Default",
        "description": "Default template with all possible settings",
        "img": "/img/default-template.gif",
        "author": "System",
        "date": "2022-01-01",
        "version": "1.0.0",
        "tags": ["basic", "all-purpose"],
        "categories": ["general"],
        "defaultSize": "1200x628",
        "size": {
            "480x120": {
                "width": 480,
                "height": 120,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {}
            },
            "300x250": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "160x600": {
                "width": 160,
                "height": 600,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "300x250-text": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "728x90": {
                "width": 728,
                "height": 90,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1200x628": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1200x628-2": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1080x1080": {
                "width": 1080,
                "height": 1080,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "720x1280": {
                "width": 720,
                "height": 1280,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            }
        }
    },
    "full-shot": {
        "id": "full-shot",
        "name": "Full shot",
        "description": "Template optimized for full-body shots",
        "img": "/img/default-template.gif",
        "author": "https://boords.com/blog/16-types-of-camera-shots-and-angles-with-gifs",
        "date": "2022-01-01",
        "version": "1.0.0",
        "tags": ["photography", "full-body"],
        "categories": ["product", "fashion"],
        "defaultSize": "1200x628",
        "size": {
            "480x120": {
                "width": 480,
                "height": 120,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "300x250": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "160x600": {
                "width": 160,
                "height": 600,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "300x250-text": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "728x90": {
                "width": 728,
                "height": 90,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1200x628": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": false,
                    "filter": false,
                    "frontdrop": false,
                    "backbanner": false,
                    "pathOne": false,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1200x628-2": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1080x1080": {
                "width": 1080,
                "height": 1080,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "720x1280": {
                "width": 720,
                "height": 1280,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            }
        }
    },
    "hero-shot": {
        "id": "hero-shot",
        "name": "Hero Shot",
        "description": "Template optimized for full-body shots",
        "img": "/img/default-template.gif",
        "author": "https://boords.com/blog/16-types-of-camera-shots-and-angles-with-gifs",
        "date": "2022-01-01",
        "version": "1.0.0",
        "tags": ["photography", "full-body"],
        "categories": ["product", "fashion"],
        "defaultSize": "1200x628",
        "size": {
            "480x120": {
                "width": 480,
                "height": 120,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "300x250": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "160x600": {
                "width": 160,
                "height": 600,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "300x250-text": {
                "width": 300,
                "height": 250,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "728x90": {
                "width": 728,
                "height": 90,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1200x628": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": false,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": false,
                    "backbanner": true,
                    "frontdrop": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": 1,
                    "svgOneOpacity": 0.53,
                    "svgTwoOpacity": 1,
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "43%",
                    "titleSize": "2.9rem",
                    "bodySize": "1.25rem",
                    "switchPosition": false,
                    "imageZIndex": 16,
                    "textLength": 0,
                    "zIndex": {
                        "image": 24
                    },
                    "logo-icon": {
                        "position": "absolute",
                        "top": "592px",
                        "left": "16px",
                        "width": "178.133px",
                        "height": "28px",
                        "fontSize": "20px",
                        "zIndex": "30",
                        "opacity": "1",
                        "transform": "matrix(1, 0, 0, 1, 0, 0)",
                        "display": "block"
                    },
                    "image": {
                        "position": "absolute",
                        "top": "56px",
                        "left": "96px",
                        "width": "516px",
                        "height": "516px",
                        "fontSize": "17.6px",
                        "zIndex": "24",
                        "opacity": "0",
                        "transform": "none",
                        "display": "block"
                    },
                    "headline": {
                        "position": "static",
                        "top": "0px",
                        "left": "0px",
                        "width": "400px",
                        "height": "120.625px",
                        "fontSize": "46.4px",
                        "zIndex": "auto",
                        "opacity": "1",
                        "transform": "none",
                        "display": "block"
                    },
                    "text": {
                        "position": "static",
                        "top": "66.6667%",
                        "left": "0px",
                        "width": "auto",
                        "height": "auto",
                        "fontSize": "20px",
                        "zIndex": "auto",
                        "opacity": "1",
                        "transform": "none",
                        "display": "none"
                    },
                    "tags": {
                        "position": "static",
                        "top": "33.3333%",
                        "left": "0px",
                        "width": "400px",
                        "height": "20px",
                        "fontSize": "14px",
                        "zIndex": "auto",
                        "opacity": "1",
                        "transform": "none",
                        "display": "block"
                    },
                    "cta": {
                        "position": "static",
                        "top": "auto",
                        "left": "0px",
                        // "width": "129.656px",
                        // "height": "30.3984px",
                        "fontSize": "17.6px",
                        "zIndex": "auto",
                        "opacity": "1",
                        "transform": "none",
                        "display": "block"
                    },
                    "path-one": {
                        "position": "static",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "0.5",
                        "transform": "none",
                        "display": "block"
                    },
                    "path-two": {
                        "position": "static",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "0.3",
                        "transform": "none",
                        "display": "block"
                    },
                    "textcontainer": {
                        "position": "absolute",
                        "top": "314px",
                        "left": "800px",
                        "width": "400px",
                        "height": "171.023px",
                        "fontSize": "17.6px",
                        "zIndex": "20",
                        "opacity": "1",
                        "transform": "matrix(1, 0, 0, 1, -64, -85.5117)",
                        "display": "block"
                    },
                    "backdrop": {
                        "position": "absolute",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "1",
                        "transform": "none",
                        "display": "none"
                    },
                    "filter": {
                        "position": "absolute",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "1",
                        "transform": "none",
                        "display": "none"
                    },
                    "frontdrop": {
                        "position": "absolute",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "1",
                        "transform": "none",
                        "display": "none"
                    },
                    "backbanner": {
                        "position": "absolute",
                        "top": "0px",
                        "left": "0px",
                        "width": "100%",
                        "height": "100%",
                        "fontSize": "17.6px",
                        "zIndex": "0",
                        "opacity": "1",
                        "transform": "none",
                        "display": "none"
                    },
                    "scales": {
                        "backdropimage": 1.331,
                        "frontdropimage": 1.1979
                    },
                    "positions": {
                        "frontdropimage": {
                            "top": "0px",
                            "left": "0px"
                        },
                        "frontdrop": {
                            "top": "0px",
                            "left": "-40px"
                        }
                    }
                }

            },
            "1200x628-2": {
                "width": 1200,
                "height": 628,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "1080x1080": {
                "width": 1080,
                "height": 1080,
                "img": "/img/default-template.gif",
                "elements": {
                    "logoIcon": true,
                    "image": true,
                    "headline": true,
                    "text": true,
                    "tags": true,
                    "cta": true,
                    "svgWave": true,
                    "textContainer": true,
                    "backdrop": true,
                    "filter": true,
                    "frontdrop": true,
                    "backbanner": true,
                    "pathOne": true,
                    "pathTwo": true,
                    "logo-title": true
                },
                "layout": {
                    "logoLayout": "flex-row",
                    "ctaStyle": "default",
                    "imageOpacity": "1",
                    "svgOneOpacity": "1",
                    "svgTwoOpacity": "1",
                    "logoRotation": 0,
                    "backgroundImage": false,
                    "imageSize": "27%",
                    "titleSize": "1.2rem",
                    "bodySize": "1rem",
                    "switchPosition": false,
                    "imageZIndex": 29,
                    "textLength": 0,
                    "zIndex": {
                        "logoIcon": 10,
                        "image": 20,
                        "headline": 30,
                        "text": 40,
                        "tags": 50,
                        "cta": 60,
                        "svgWave": 5,
                        "textContainer": 15,
                        "backdrop": 1,
                        "filter": 2,
                        "frontdrop": 3,
                        "backbanner": 0,
                        "pathOne": 6,
                        "pathTwo": 7,
                        "logo-title": 11
                    },
                    "scales": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "positions": {
                        "logoIcon": { "top": "0px", "left": "0px" },
                        "image": { "top": "0px", "left": "125px" },
                        "headline": { "top": "30px", "left": "0px" },
                        "text": { "top": "60px", "left": "0px" },
                        "tags": { "top": "-25px", "left": "0px" },
                        "cta": { "top": "-45px", "left": "230px" },
                        "svgWave": { "top": "0px", "left": "0px" },
                        "textContainer": { "top": "0px", "left": "0px" },
                        "backdrop": { "top": "0px", "left": "0px" },
                        "filter": { "top": "0px", "left": "0px" },
                        "frontdrop": { "top": "0px", "left": "0px" },
                        "backbanner": { "top": "0px", "left": "0px" },
                        "pathOne": { "top": "0px", "left": "0px" },
                        "pathTwo": { "top": "0px", "left": "0px" },
                        "logo-title": { "top": "0px", "left": "0px" }
                    },
                    "colors": {
                        "logoIcon": "#000000",
                        "image": "#FFFFFF",
                        "headline": "#000000",
                        "text": "#000000",
                        "tags": "#000000",
                        "cta": "#FFFFFF",
                        "svgWave": "#000000",
                        "textContainer": "#FFFFFF",
                        "backdrop": "#FFFFFF",
                        "filter": "#FFFFFF",
                        "frontdrop": "#FFFFFF",
                        "backbanner": "#FFFFFF",
                        "pathOne": "#000000",
                        "pathTwo": "#000000",
                        "logo-title": "#000000"
                    },
                    "opacities": {
                        "logoIcon": 1,
                        "image": 1,
                        "headline": 1,
                        "text": 1,
                        "tags": 1,
                        "cta": 1,
                        "svgWave": 1,
                        "textContainer": 1,
                        "backdrop": 1,
                        "filter": 1,
                        "frontdrop": 1,
                        "backbanner": 1,
                        "pathOne": 1,
                        "pathTwo": 1,
                        "logo-title": 1
                    },
                    "rotations": {
                        "logoIcon": 0,
                        "image": 0,
                        "headline": 0,
                        "text": 0,
                        "tags": 0,
                        "cta": 0,
                        "svgWave": 0,
                        "textContainer": 0,
                        "backdrop": 0,
                        "filter": 0,
                        "frontdrop": 0,
                        "backbanner": 0,
                        "pathOne": 0,
                        "pathTwo": 0,
                        "logo-title": 0
                    },
                    "filters": {
                        "logoIcon": "none",
                        "image": "none",
                        "headline": "none",
                        "text": "none",
                        "tags": "none",
                        "cta": "none",
                        "svgWave": "none",
                        "textContainer": "none",
                        "backdrop": "none",
                        "filter": "none",
                        "frontdrop": "none",
                        "backbanner": "none",
                        "pathOne": "none",
                        "pathTwo": "none",
                        "logo-title": "none"
                    }
                }
            },
            "720x1280": {
                "width": 720,
                "height": 1280,
                "img": "/img/default-template.gif",
                "elements": {
                  "logoIcon": true,
                  "image": true,
                  "headline": true,
                  "text": true,
                  "tags": true,
                  "cta": true,
                  "svgWave": true,
                  "textContainer": true,
                  "backdrop": true,
                  "filter": true,
                  "frontdrop": true,
                  "backbanner": true,
                  "pathOne": true,
                  "pathTwo": true,
                  "logo-title": true
                },
                "layout": {
                  "logoLayout": "flex-row",
                  "ctaStyle": "default",
                  "imageOpacity": "1",
                  "svgOneOpacity": "1",
                  "svgTwoOpacity": "1",
                  "logoRotation": 0,
                  "backgroundImage": false,
                  "imageSize": "27%",
                  "titleSize": "1.2rem",
                  "bodySize": "1rem",
                  "switchPosition": false,
                  "imageZIndex": 29,
                  "textLength": 0,
                  "zIndex": {
                    "logoIcon": 10,
                    "image": 20,
                    "headline": 30,
                    "text": 40,
                    "tags": 50,
                    "cta": 60,
                    "svgWave": 5,
                    "textContainer": 15,
                    "backdrop": 1,
                    "filter": 2,
                    "frontdrop": 3,
                    "backbanner": 0,
                    "pathOne": 6,
                    "pathTwo": 7,
                    "logo-title": 11
                  },
                  "scales": {
                    "logoIcon": 1,
                    "image": 1,
                    "headline": 1,
                    "text": 1,
                    "tags": 1,
                    "cta": 1,
                    "svgWave": 1,
                    "textContainer": 1,
                    "backdrop": 1,
                    "filter": 1,
                    "frontdrop": 1,
                    "backbanner": 1,
                    "pathOne": 1,
                    "pathTwo": 1,
                    "logo-title": 1
                  },
                  "positions": {
                    "logoIcon": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "image": {
                      "top": "0px",
                      "left": "125px"
                    },
                    "headline": {
                      "top": "30px",
                      "left": "0px"
                    },
                    "text": {
                      "top": "60px",
                      "left": "0px"
                    },
                    "tags": {
                      "top": "-25px",
                      "left": "0px"
                    },
                    "cta": {
                      "top": "-45px",
                      "left": "230px"
                    },
                    "svgWave": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "textContainer": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "backdrop": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "filter": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "frontdrop": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "backbanner": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "pathOne": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "pathTwo": {
                      "top": "0px",
                      "left": "0px"
                    },
                    "logo-title": {
                      "top": "0px",
                      "left": "0px"
                    }
                  },
                  "colors": {
                    "logoIcon": "#000000",
                    "image": "#FFFFFF",
                    "headline": "#000000",
                    "text": "#000000",
                    "tags": "#000000",
                    "cta": "#FFFFFF",
                    "svgWave": "#000000",
                    "textContainer": "#FFFFFF",
                    "backdrop": "#FFFFFF",
                    "filter": "#FFFFFF",
                    "frontdrop": "#FFFFFF",
                    "backbanner": "#FFFFFF",
                    "pathOne": "#000000",
                    "pathTwo": "#000000",
                    "logo-title": "#000000"
                  },
                  "opacities": {
                    "logoIcon": 1,
                    "image": 1,
                    "headline": 1,
                    "text": 1,
                    "tags": 1,
                    "cta": 1,
                    "svgWave": 1,
                    "textContainer": 1,
                    "backdrop": 1,
                    "filter": 1,
                    "frontdrop": 1,
                    "backbanner": 1,
                    "pathOne": 1,
                    "pathTwo": 1,
                    "logo-title": 1
                  },
                  "rotations": {
                    "logoIcon": 0,
                    "image": 0,
                    "headline": 0,
                    "text": 0,
                    "tags": 0,
                    "cta": 0,
                    "svgWave": 0,
                    "textContainer": 0,
                    "backdrop": 0,
                    "filter": 0,
                    "frontdrop": 0,
                    "backbanner": 0,
                    "pathOne": 0,
                    "pathTwo": 0,
                    "logo-title": 0
                  },
                  "filters": {
                    "logoIcon": "none",
                    "image": "none",
                    "headline": "none",
                    "text": "none",
                    "tags": "none",
                    "cta": "none",
                    "svgWave": "none",
                    "textContainer": "none",
                    "backdrop": "none",
                    "filter": "none",
                    "frontdrop": "none",
                    "backbanner": "none",
                    "pathOne": "none",
                    "pathTwo": "none",
                    "logo-title": "none"
                  }
                }
              }
        }
    }
}

let settingsTemplates_ = [
    {
        "id": "default",
        "name": "Default",
        "description": "Default template with all possible settings",
        "img": "/img/default-template.gif",
        "width": 480,
        "height": 120,
        "date": "2023-07-29",
        "author": "System",
        "elements": {
            "logoIcon": true,
            "image": true,
            "headline": true,
            "text": true,
            "tags": true,
            "cta": true,
            "svgWave": true,
            "textContainer": true,
            "backdrop": true,
            "filter": true,
            "frontdrop": true,
            "backbanner": true,
            "pathOne": true,
            "pathTwo": true,
            "logo-title": true
        },
        "layout": {
            "logoLayout": "flex-row",
            "ctaStyle": "default",
            "imageOpacity": "1",
            "svgOneOpacity": "1",
            "svgTwoOpacity": "1",
            "logoRotation": 0,
            "backgroundImage": false,
            "imageSize": "27%",
            "titleSize": "1.2rem",
            "bodySize": "1rem",
            "switchPosition": false,
            "imageZIndex": 29,
            "textLength": 0,
            "zIndex": {
                "logoIcon": 10,
                "image": 20,
                "headline": 30,
                "text": 40,
                "tags": 50,
                "cta": 60,
                "svgWave": 5,
                "textContainer": 15,
                "backdrop": 1,
                "filter": 2,
                "frontdrop": 3,
                "backbanner": 0,
                "pathOne": 6,
                "pathTwo": 7,
                "logo-title": 11
            },
            "scales": {
                "logoIcon": 1,
                "image": 1,
                "headline": 1,
                "text": 1,
                "tags": 1,
                "cta": 1,
                "svgWave": 1,
                "textContainer": 1,
                "backdrop": 1,
                "filter": 1,
                "frontdrop": 1,
                "backbanner": 1,
                "pathOne": 1,
                "pathTwo": 1,
                "logo-title": 1
            },
            "positions": {
                "logoIcon": { "top": "0px", "left": "0px" },
                "image": { "top": "0px", "left": "125px" },
                "headline": { "top": "30px", "left": "0px" },
                "text": { "top": "60px", "left": "0px" },
                "tags": { "top": "-25px", "left": "0px" },
                "cta": { "top": "-45px", "left": "230px" },
                "svgWave": { "top": "0px", "left": "0px" },
                "textContainer": { "top": "0px", "left": "0px" },
                "backdrop": { "top": "0px", "left": "0px" },
                "filter": { "top": "0px", "left": "0px" },
                "frontdrop": { "top": "0px", "left": "0px" },
                "backbanner": { "top": "0px", "left": "0px" },
                "pathOne": { "top": "0px", "left": "0px" },
                "pathTwo": { "top": "0px", "left": "0px" },
                "logo-title": { "top": "0px", "left": "0px" }
            },
            "colors": {
                "logoIcon": "#000000",
                "image": "#FFFFFF",
                "headline": "#000000",
                "text": "#000000",
                "tags": "#000000",
                "cta": "#FFFFFF",
                "svgWave": "#000000",
                "textContainer": "#FFFFFF",
                "backdrop": "#FFFFFF",
                "filter": "#FFFFFF",
                "frontdrop": "#FFFFFF",
                "backbanner": "#FFFFFF",
                "pathOne": "#000000",
                "pathTwo": "#000000",
                "logo-title": "#000000"
            },
            "opacities": {
                "logoIcon": 1,
                "image": 1,
                "headline": 1,
                "text": 1,
                "tags": 1,
                "cta": 1,
                "svgWave": 1,
                "textContainer": 1,
                "backdrop": 1,
                "filter": 1,
                "frontdrop": 1,
                "backbanner": 1,
                "pathOne": 1,
                "pathTwo": 1,
                "logo-title": 1
            },
            "rotations": {
                "logoIcon": 0,
                "image": 0,
                "headline": 0,
                "text": 0,
                "tags": 0,
                "cta": 0,
                "svgWave": 0,
                "textContainer": 0,
                "backdrop": 0,
                "filter": 0,
                "frontdrop": 0,
                "backbanner": 0,
                "pathOne": 0,
                "pathTwo": 0,
                "logo-title": 0
            },
            "filters": {
                "logoIcon": "none",
                "image": "none",
                "headline": "none",
                "text": "none",
                "tags": "none",
                "cta": "none",
                "svgWave": "none",
                "textContainer": "none",
                "backdrop": "none",
                "filter": "none",
                "frontdrop": "none",
                "backbanner": "none",
                "pathOne": "none",
                "pathTwo": "none",
                "logo-title": "none"
            }
        }
    }, {
        "id": "establishing-shot",
        "name": "Establishing shot",
        "description": "this is a description for the settings template",
        "img": "/img/establishing-shot.gif",
        "width": 480,
        "height": 120,
        "date": "2022-01-01",
        "author": "john doe",

        "elements": {
            "logoIcon": true,
            "image": true,
            "headline": true,
            "text": false,
            "tags": true,
            "cta": true,
            "svgWave": true,
            "textContainer": true,
            "backdrop": false,
            "filter": false,
            "backbanner": false
        },
        "layout": {
            "logoLayout": "flex-row",
            "ctaStyle": "circle",
            "imageOpacity": "1",
            "svgOneOpacity": "1",
            "svgTwoOpacity": "1",
            "logoRotation": 0,
            "backgroundImage": false,
            "imageSize": "27%",
            "titleSize": "1.2rem",
            "bodySize": "1rem",
            "switchPosition": false,
            "imageZIndex": 29,
            "textLength": 0,
            "zIndex": {
                "backdropimage": 10
            },
            "scales": {
                "backdropimage": 0.9900000000000001
            },
            "positions": {
                "logo": {
                    "top": "0px",
                    "left": "-5px"
                },
                "headline": {
                    "top": "30px",
                    "left": "0px"
                },
                "image": {
                    "top": "0px",
                    "left": "125px"
                },
                "cta": {
                    "top": "-45px",
                    "left": "230px"
                },
                "tags": {
                    "top": "-25px",
                    "left": "0px"
                }
            }
        }

    },
    {
        "id": "full-shot",
        "name": "Full shot",
        "description": "this is a description for the settings template",
        "width": 480,
        "height": 120,
        "date": "2022-01-01",
        "author": "https://boords.com/blog/16-types-of-camera-shots-and-angles-with-gifs",

        "elements": {
            "logoIcon": true,
            "image": true,
            "headline": true,
            "text": false,
            "tags": true,
            "cta": true,
            "svgWave": true,
            "textContainer": true,
            "backdrop": false,
            "filter": false,
            "backbanner": false
        },
        "layout": {
            "logoLayout": "flex-row",
            "ctaStyle": "circle",
            "imageOpacity": "1",
            "svgOneOpacity": "1",
            "svgTwoOpacity": "1",
            "logoRotation": 0,
            "backgroundImage": false,
            "imageSize": "27%",
            "titleSize": "1.2rem",
            "bodySize": "1rem",
            "switchPosition": false,
            "imageZIndex": 29,
            "textLength": 0,
            "zIndex": {
                "backdropimage": 10
            },
            "scales": {
                "backdropimage": 0.9900000000000001
            },
            "positions": {
                "logo": {
                    "top": "0px",
                    "left": "-5px"
                },
                "headline": {
                    "top": "30px",
                    "left": "0px"
                },
                "image": {
                    "top": "0px",
                    "left": "125px"
                },
                "cta": {
                    "top": "-45px",
                    "left": "230px"
                },
                "tags": {
                    "top": "-25px",
                    "left": "0px"
                }
            }
        }

    }
]

const bannerTemplates = [
    {
        "id": "480x120",
        "name": "480x120",
        "width": 480,
        "height": 120,
        "path": "/img/banners/480x120/",
        "direction": "horizontal",
        "description": "this is a description for the banner template",
        "images": [
            "banner-01.jpg",
            "banner-02.jpg",
            "banner-03.jpg",
            "banner-04.jpg",
            "banner-05.jpg",
            "banner-06.jpg",
            "banner-07.jpg",
            "banner-08.jpg",
            "banner-09.jpg",
            "banner-10.jpg"
        ],
        "tags": ["#banner", "#480x120", "#marketing"],
        "category": "marketing",
        "type": "banner",
        "date": "2022-01-01",
        "author": "john doe",

        "cta": "Learn More",
        "ctaUrl": "#",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "animation": {
            "isLooped": false,
            "isExitAnimated": true,
            "isEntryAnimated": true,
            "template": "fade-in"
        }
    },
]

const campaigns_design_x = [
    {
        "title": "How to enrich your business?",
        "description": "this is a description",
        "img": "/img/leads.png",
        "parentUrl": "/blog/leads",
        "tags": ["#leads", "#business"],
        "category": "business",
        "date": "2022-01-01",
        "author": "john doe",
        "type": "blog",
        "audio": [
            {
                "url": "/audio/01-pod-cast.aac",
                "duration": "04:50:000"
            },
            {
                "url": "/audio/01-pod-cast.aac",
                "duration": "04:50:000"
            }
        ],
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "ads": [
            {
                "timestamp": "00:00:000",
                "audio": 0,
                "visuals": [
                    {
                        "id": "unique-id",
                        "type": "image",
                        "src": "/img/leads.png",
                        "alt": "leads",
                        "settings": {
                            "width": "100%",
                            "height": "100%",
                            "object-fit": "cover"
                        }

                    }
                ],
                "headline": "Podcast Starts",
                "text": ["Welcome back to another episode of Marketing Insights, the podcast where we explore the latest trends, challenges, and opportunities in the world of marketing and business growth. I'm your host, Mike Sanshase, and today, we have a special guest with us, Sherif Butt, who has over 10 years of experience in the field. Welcome, Sherif!"],
                "tags": "#MarketingInsights #PodcastIntro",
                "img": "/img/episode_start.webp",
                "filter": "/img/episode_start.webp",
                "imgDescription": "A 3D sketch style drawing of a podcast studio viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The podcast studio should appear modern and abstract, set against a strictly white background.",
                "cta": "Stay Tuned",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "fade-in"
                }
            },
            {
                "timestamp": "00:10:000",
                "audio": 0,
                "headline": "Guest Introduction",
                "text": ["Thanks, Mike! It's great to be here."],
                "tags": "#MarketingInsights #GuestIntro",
                "img": "/img/guest_intro.webp",
                "imgDescription": "A 3D sketch style drawing of a guest speaker viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The guest speaker should appear modern and abstract, set against a strictly white background.",
                "cta": "Meet Sherif",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-some-to-right"
                }
            },
            {
                "timestamp": "00:15:000",
                "audio": 0,
                "headline": "Lead Generation Hesitation",
                "text": ["Today, we're diving into a topic that many businesses grapple with: Why Businesses Hesitate to Implement Lead Generation. Sherif, you've worked with numerous businesses over the years. Why do you think there's so much hesitation around lead generation?"],
                "tags": "#LeadGeneration #Marketing",
                "img": "/img/lead_generation.webp",
                "imgDescription": "A 3D sketch style drawing of a business team discussing strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Discover More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-to-right",
                }
            },
            {
                "timestamp": "00:30:000",
                "audio": 0,
                "headline": "Misconceptions",
                "text": ["That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective. While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming."],
                "tags": "#LeadGeneration #Misconceptions",
                "img": "/img/misconceptions.webp",
                "imgDescription": "A 3D sketch style drawing of a confused businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "00:50:000",
                "audio": 0,
                "headline": "Immediate Results",
                "text": ["That makes sense. So, what would you say to businesses that believe lead generation doesn't yield immediate results?"],
                "tags": "#LeadGeneration #Results",
                "img": "/img/immediate_results.webp",
                "imgDescription": "A 3D sketch style drawing of a graph showing rapid results viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
                "cta": "See Results",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:00:000",
                "audio": 0,
                "headline": "Effective Strategies",
                "text": ["It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly. The key is to set realistic expectations and have a solid plan."],
                "tags": "#LeadGeneration #Strategies",
                "img": "/img/effective_strategies.webp",
                "imgDescription": "A 3D sketch style drawing of a strategic plan viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The strategic plan should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:20:000",
                "audio": "/audio/07-pod-cast.aac",
                "headline": "Audience Understanding",
                "text": ["Absolutely. Now, let's talk about some of the challenges businesses face when implementing lead generation. What are some of the common obstacles?"],
                "tags": "#LeadGeneration #Challenges",
                "img": "/img/audience_understanding.webp",
                "imgDescription": "A 3D sketch style drawing of a business meeting viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Face Challenges",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:30:000",
                "audio": 0,
                "headline": "Common Obstacles",
                "text": ["One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected, leading to frustration and wasted resources."],
                "tags": "#LeadGeneration #Obstacles",
                "img": "/img/common_obstacles.webp",
                "imgDescription": "A 3D sketch style drawing of a frustrated businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Overcome Obstacles",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:50:000",
                "audio": 0,
                "headline": "Fear of the Unknown",
                "text": ["And what about the fear of the unknown? I've heard that many businesses simply don't know where to start."],
                "tags": "#LeadGeneration #Fear",
                "img": "/img/fear_of_unknown.webp",
                "imgDescription": "A 3D sketch style drawing of a confused business team viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Start Here",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:00:000",
                "audio": 0,
                "headline": "Expert Guidance",
                "text": ["That's a significant barrier, Mike. Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support to get started on the right foot."],
                "tags": "#LeadGeneration #ExpertGuidance",
                "img": "/img/expert_guidance.webp",
                "imgDescription": "A 3D sketch style drawing of a business consultation viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business consultation should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Support",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:20:000",
                "audio": 0,
                "headline": "Practical Solutions",
                "text": ["Great points, Sherif. So, what are some practical solutions to overcome these barriers and successfully implement lead generation?"],
                "tags": "#LeadGeneration #Solutions",
                "img": "/img/practical_solutions.webp",
                "imgDescription": "A 3D sketch style drawing of a business meeting discussing solutions viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Find Solutions",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:30:000",
                "audio": 0,
                "headline": "Education and Training",
                "text": ["First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices. There are numerous online courses, webinars, and resources available that can help demystify the process."],
                "tags": "#LeadGeneration #Education",
                "img": "/img/education_training.webp",
                "imgDescription": "A 3D sketch style drawing of a business training session viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The training session should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:50:000",
                "audio": 0,
                "headline": "Start Small",
                "text": ["Starting small and scaling up can also be effective, right?"],
                "tags": "#LeadGeneration #StartSmall",
                "img": "/img/start_small.webp",
                "imgDescription": "A 3D sketch style drawing of a small business team working on a project viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The small business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Begin Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:00:000",
                "audio": 0,
                "headline": "Effective Strategies",
                "text": ["Exactly, Mike. Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."],
                "tags": "#LeadGeneration #EffectiveStrategies",
                "img": "/img/effective_strategies_2.webp",
                "imgDescription": "A 3D sketch style drawing of a business team planning strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:20:000",
                "audio": 0,
                "headline": "Leverage Technology",
                "text": ["And technology plays a big role too, I assume?"],
                "tags": "#LeadGeneration #Technology",
                "img": "/img/technology.webp",
                "imgDescription": "A 3D sketch style drawing of technology tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The technology tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Use Tech",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:30:000",
                "audio": 0,
                "headline": "Tech for Optimization",
                "text": ["Absolutely. There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."],
                "tags": "#LeadGeneration #Optimization",
                "img": "/img/tech_optimization.webp",
                "imgDescription": "A 3D sketch style drawing of CRM and email marketing tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Optimize Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:50:000",
                "audio": 0,
                "headline": "Expert Partnership",
                "text": ["What about partnering with experts? How can that help?"],
                "tags": "#LeadGeneration #ExpertPartnership",
                "img": "/img/expert_partnership.webp",
                "imgDescription": "A 3D sketch style drawing of a business partnership viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business partnership should appear modern and abstract, set against a strictly white background.",
                "cta": "Partner Up",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:00:000",
                "audio": 0,
                "headline": "Get Expert Help",
                "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference. They bring the expertise and experience needed to navigate the complexities of lead generation and ensure successful implementation."],
                "tags": "#LeadGeneration #ExpertHelp",
                "img": "/img/get_expert_help.webp",
                "imgDescription": "A 3D sketch style drawing of a business expert providing guidance viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business expert should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Help",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:20:000",
                "audio": 0,
                "headline": "Conclusion",
                "text": ["Thanks for those insights, Sherif. In conclusion, while businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. By educating themselves, starting small, leveraging technology, and seeking expert guidance, businesses can overcome these barriers and unlock the full potential of lead generation."],
                "tags": "#LeadGeneration #Conclusion",
                "img": "/img/conclusion.webp",
                "imgDescription": "A 3D sketch style drawing of a business team celebrating success viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Started",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:40:000",
                "audio": 0,
                "headline": "Thank You",
                "text": ["Exactly, Mike. Lead generation can truly transform a business when done right."],
                "tags": "#LeadGeneration #ThankYou",
                "img": "/img/thank_you.webp",
                "imgDescription": "A 3D sketch style drawing of a thank you message viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The thank you message should appear modern and abstract, set against a strictly white background.",
                "cta": "Thank You",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:50:000",
                "audio": 0,
                "headline": "Final Words",
                "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. And don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."],
                "tags": "#LeadGeneration #FinalWords",
                "img": "/img/final_words.webp",
                "imgDescription": "A 3D sketch style drawing of a business handshake viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business handshake should appear modern and abstract, set against a strictly white background.",
                "cta": "Subscribe",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            }
        ]





    }
]
const campaigns_05_x = [
    {
        "title": "how to inhace your business",
        "description": "this is a description",
        "img": "/img/leads.png",
        "parentUrl": "/blog/leads",
        "tags": ["#leads", "#business"],
        "category": "business",
        "date": "2022-01-01",
        "author": "john doe",
        "type": "blog",
        "audio": [{
            "url": "/audio/01-pod-cast.aac",
            "duration": "04:50:000",
        },
        {
            "url": "/audio/02-pod-cast.aac",
            "duration": "04:50:000",
        },
        ],
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "ads": [
            {
                "timestamp": "00:00:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Podcast Starts",
                "text": ["Welcome back to another episode of Marketing Insights, the podcast where we explore the latest trends, challenges, and opportunities in the world of marketing and business growth. I'm your host, Mike Sanshase, and today, we have a special guest with us, Sherif Butt, who has over 10 years of experience in the field. Welcome, Sherif!"],
                "tags": "#MarketingInsights #PodcastIntro",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a podcast studio viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The podcast studio should appear modern and abstract, set against a strictly white background.",
                "cta": "Stay Tuned",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "fade-in"
                }
            },
            {
                "timestamp": "00:10:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Guest Introduction",
                "text": ["Thanks, Mike! It's great to be here."],
                "tags": "#MarketingInsights #GuestIntro",
                "img": "/img/guest_intro.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a guest speaker viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The guest speaker should appear modern and abstract, set against a strictly white background.",
                "cta": "Meet Sherif",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-some-to-right"
                }
            },
            {
                "timestamp": "00:15:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Lead Generation Hesitation",
                "text": ["Today, we're diving into a topic that many businesses grapple with: Why Businesses Hesitate to Implement Lead Generation. Sherif, you've worked with numerous businesses over the years. Why do you think there's so much hesitation around lead generation?"],
                "tags": "#LeadGeneration #Marketing",
                "img": "/img/lead_generation.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team discussing strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Discover More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-to-right",
                }
            },
            {
                "timestamp": "00:30:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Misconceptions",
                "text": ["That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective. While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming."],
                "tags": "#LeadGeneration #Misconceptions",
                "img": "/img/misconceptions.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "00:50:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Immediate Results",
                "text": ["That makes sense. So, what would you say to businesses that believe lead generation doesn't yield immediate results?"],
                "tags": "#LeadGeneration #Results",
                "img": "/img/immediate_results.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a graph showing rapid results viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
                "cta": "See Results",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:00:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly. The key is to set realistic expectations and have a solid plan."],
                "tags": "#LeadGeneration #Strategies",
                "img": "/img/effective_strategies.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a strategic plan viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The strategic plan should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:20:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Audience Understanding",
                "text": ["Absolutely. Now, let's talk about some of the challenges businesses face when implementing lead generation. What are some of the common obstacles?"],
                "tags": "#LeadGeneration #Challenges",
                "img": "/img/audience_understanding.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Face Challenges",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:30:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Common Obstacles",
                "text": ["One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected, leading to frustration and wasted resources."],
                "tags": "#LeadGeneration #Obstacles",
                "img": "/img/common_obstacles.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a frustrated businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Overcome Obstacles",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:50:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Fear of the Unknown",
                "text": ["And what about the fear of the unknown? I've heard that many businesses simply don't know where to start."],
                "tags": "#LeadGeneration #Fear",
                "img": "/img/fear_of_unknown.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused business team viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Start Here",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:00:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Guidance",
                "text": ["That's a significant barrier, Mike. Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support to get started on the right foot."],
                "tags": "#LeadGeneration #ExpertGuidance",
                "img": "/img/expert_guidance.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business consultation viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business consultation should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Support",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:20:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Practical Solutions",
                "text": ["Great points, Sherif. So, what are some practical solutions to overcome these barriers and successfully implement lead generation?"],
                "tags": "#LeadGeneration #Solutions",
                "img": "/img/practical_solutions.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting discussing solutions viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Find Solutions",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:30:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Education and Training",
                "text": ["First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices. There are numerous online courses, webinars, and resources available that can help demystify the process."],
                "tags": "#LeadGeneration #Education",
                "img": "/img/education_training.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business training session viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The training session should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:50:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Start Small",
                "text": ["Starting small and scaling up can also be effective, right?"],
                "tags": "#LeadGeneration #StartSmall",
                "img": "/img/start_small.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a small business team working on a project viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The small business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Begin Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:00:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["Exactly, Mike. Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."],
                "tags": "#LeadGeneration #EffectiveStrategies",
                "img": "/img/effective_strategies_2.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team planning strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:20:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Leverage Technology",
                "text": ["And technology plays a big role too, I assume?"],
                "tags": "#LeadGeneration #Technology",
                "img": "/img/technology.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of technology tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The technology tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Use Tech",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:30:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Tech for Optimization",
                "text": ["Absolutely. There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."],
                "tags": "#LeadGeneration #Optimization",
                "img": "/img/tech_optimization.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of CRM and email marketing tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Optimize Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:50:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Partnership",
                "text": ["What about partnering with experts? How can that help?"],
                "tags": "#LeadGeneration #ExpertPartnership",
                "img": "/img/expert_partnership.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business partnership viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business partnership should appear modern and abstract, set against a strictly white background.",
                "cta": "Partner Up",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:00:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Get Expert Help",
                "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference. They bring the expertise and experience needed to navigate the complexities of lead generation and ensure successful implementation."],
                "tags": "#LeadGeneration #ExpertHelp",
                "img": "/img/get_expert_help.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business expert providing guidance viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business expert should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Help",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:20:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Conclusion",
                "text": ["Thanks for those insights, Sherif. In conclusion, while businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. By educating themselves, starting small, leveraging technology, and seeking expert guidance, businesses can overcome these barriers and unlock the full potential of lead generation."],
                "tags": "#LeadGeneration #Conclusion",
                "img": "/img/conclusion.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team celebrating success viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Started",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:40:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Thank You",
                "text": ["Exactly, Mike. Lead generation can truly transform a business when done right."],
                "tags": "#LeadGeneration #ThankYou",
                "img": "/img/thank_you.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a thank you message viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The thank you message should appear modern and abstract, set against a strictly white background.",
                "cta": "Thank You",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:50:000",
                "audio": 0,
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Final Words",
                "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. And don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."],
                "tags": "#LeadGeneration #FinalWords",
                "img": "/img/final_words.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business handshake viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business handshake should appear modern and abstract, set against a strictly white background.",
                "cta": "Subscribe",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            }
        ]





    }
]
const campaigns = [
    {
        "title": "how to inhace your business",
        "description": "this is a description",
        "img": "/img/leads.png",
        "parentUrl": "/blog/leads",
        "tags": [
            "#leads",
            "#business"
        ],
        "category": "business",
        "date": "2022-01-01",
        "author": "john doe",
        "type": "blog",
        "audio": {
            "url": "/audio/01-pod-cast.aac",
            "duration": "04:50"
        },
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "slides": [
            {
                "timestamp": "00:00",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Podcast Starts",
                "text": [
                    "Welcome back to another episode of Marketing Insights, the podcast where we explore the latest trends, challenges, and opportunities in the world of marketing and business growth. I'm your host, Mike Sanshase, and today, we have a special guest with us, Sherif Butt, who has over 10 years of experience in the field. Welcome, Sherif!"
                ],
                "tags": "#MarketingInsights #PodcastIntro",
                "img": "/img/clock.webp",
                "filter": "/img/filter-32499109_7963948.png",
                "backdrop": "/img/backdrop-studio-inside-04.png",
                "frontdrop": "/img/frontdrop-studio-inside_04_n.png",
                "imgDescription": "A 3D sketch style drawing of a podcast studio viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The podcast studio should appear modern and abstract, set against a strictly white background.",
                "cta": "Stay Tuned",
                "animation": {
                    "template": "fade-in",
                    "isEntryAnimated": true,
                    "isExitAnimated": true
                }, "settingsTemplate": ""
            },
            {
                "timestamp": "00:10",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Guest Introduction",
                "text": [
                    "Thanks, Mike! It's great to be here."
                ],
                "tags": "#MarketingInsights #GuestIntro",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a guest speaker viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The guest speaker should appear modern and abstract, set against a strictly white background.",
                "cta": "Meet Sherif",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "fade-in"
                }, "settingsTemplate": "full-shot"
            },
            {
                "timestamp": "00:15",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Lead Generation Hesitation",
                "text": [
                    "Today, we're diving into a topic that many businesses grapple with: Why Businesses Hesitate to Implement Lead Generation. Sherif, you've worked with numerous businesses over the years. Why do you think there's so much hesitation around lead generation?"
                ],
                "tags": "#LeadGeneration #Marketing",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team discussing strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Discover More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-to-right"
                }
            },
            {
                "timestamp": "00:30",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Misconceptions",
                "text": [
                    "That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective. While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming."
                ],
                "tags": "#LeadGeneration #Misconceptions",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "00:50",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Immediate Results",
                "text": [
                    "That makes sense. So, what would you say to businesses that believe lead generation doesn't yield immediate results?"
                ],
                "tags": "#LeadGeneration #Results",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a graph showing rapid results viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
                "cta": "See Results",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "01:00",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": [
                    "It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly. The key is to set realistic expectations and have a solid plan."
                ],
                "tags": "#LeadGeneration #Strategies",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a strategic plan viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The strategic plan should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "01:20",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Audience Understanding",
                "text": [
                    "Absolutely. Now, let's talk about some of the challenges businesses face when implementing lead generation. What are some of the common obstacles?"
                ],
                "tags": "#LeadGeneration #Challenges",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Face Challenges",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "01:30",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Common Obstacles",
                "text": [
                    "One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected, leading to frustration and wasted resources."
                ],
                "tags": "#LeadGeneration #Obstacles",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a frustrated businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Overcome Obstacles",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "01:50",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Fear of the Unknown",
                "text": [
                    "And what about the fear of the unknown? I've heard that many businesses simply don't know where to start."
                ],
                "tags": "#LeadGeneration #Fear",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused business team viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Start Here",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "02:00",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Guidance",
                "text": [
                    "That's a significant barrier, Mike. Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support to get started on the right foot."
                ],
                "tags": "#LeadGeneration #ExpertGuidance",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business consultation viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business consultation should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Support",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "02:20",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Practical Solutions",
                "text": [
                    "Great points, Sherif. So, what are some practical solutions to overcome these barriers and successfully implement lead generation?"
                ],
                "tags": "#LeadGeneration #Solutions",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting discussing solutions viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Find Solutions",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "02:30",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Education and Training",
                "text": [
                    "First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices. There are numerous online courses, webinars, and resources available that can help demystify the process."
                ],
                "tags": "#LeadGeneration #Education",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business training session viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The training session should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "02:50",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Start Small",
                "text": [
                    "Starting small and scaling up can also be effective, right?"
                ],
                "tags": "#LeadGeneration #StartSmall",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a small business team working on a project viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The small business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Begin Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "03:00",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": [
                    "Exactly, Mike. Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."
                ],
                "tags": "#LeadGeneration #EffectiveStrategies",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team planning strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "03:20",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Leverage Technology",
                "text": [
                    "And technology plays a big role too, I assume?"
                ],
                "tags": "#LeadGeneration #Technology",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of technology tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The technology tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Use Tech",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "03:30",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Tech for Optimization",
                "text": [
                    "Absolutely. There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."
                ],
                "tags": "#LeadGeneration #Optimization",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of CRM and email marketing tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Optimize Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "03:50",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Partnership",
                "text": [
                    "What about partnering with experts? How can that help?"
                ],
                "tags": "#LeadGeneration #ExpertPartnership",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business partnership viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business partnership should appear modern and abstract, set against a strictly white background.",
                "cta": "Partner Up",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "04:00",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Get Expert Help",
                "text": [
                    "Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference. They bring the expertise and experience needed to navigate the complexities of lead generation and ensure successful implementation."
                ],
                "tags": "#LeadGeneration #ExpertHelp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business expert providing guidance viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business expert should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Help",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "04:20",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Conclusion",
                "text": [
                    "Thanks for those insights, Sherif. In conclusion, while businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. By educating themselves, starting small, leveraging technology, and seeking expert guidance, businesses can overcome these barriers and unlock the full potential of lead generation."
                ],
                "tags": "#LeadGeneration #Conclusion",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team celebrating success viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Started",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "04:40",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Thank You",
                "text": [
                    "Exactly, Mike. Lead generation can truly transform a business when done right."
                ],
                "tags": "#LeadGeneration #ThankYou",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a thank you message viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The thank you message should appear modern and abstract, set against a strictly white background.",
                "cta": "Thank You",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            },
            {
                "timestamp": "04:50",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Final Words",
                "text": [
                    "Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. And don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."
                ],
                "tags": "#LeadGeneration #FinalWords",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business handshake viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business handshake should appear modern and abstract, set against a strictly white background.",
                "cta": "Subscribe",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left"
                }
            }
        ]
    }
]
const campaigns_valid = [
    {
        "title": "how to inhace your business",
        "description": "this is a description",
        "img": "/img/leads.png",
        "parentUrl": "/blog/leads",
        "tags": ["#leads", "#business"],
        "category": "business",
        "date": "2022-01-01",
        "author": "john doe",
        "type": "blog",
        "audio": {
            "url": "/audio/01-pod-cast.aac",
            "duration": "04:50:000",
        },
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "ads": [
            {
                "timestamp": "00:00:000",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Podcast Starts",
                "text": ["Welcome back to another episode of Marketing Insights, the podcast where we explore the latest trends, challenges, and opportunities in the world of marketing and business growth. I'm your host, Mike Sanshase, and today, we have a special guest with us, Sherif Butt, who has over 10 years of experience in the field. Welcome, Sherif!"],
                "tags": "#MarketingInsights #PodcastIntro",
                "img": "/img/episode_start.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a podcast studio viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The podcast studio should appear modern and abstract, set against a strictly white background.",
                "cta": "Stay Tuned",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "fade-in"
                }
            },
            {
                "timestamp": "00:10:000",
                "audio": "/audio/02-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Guest Introduction",
                "text": ["Thanks, Mike! It's great to be here."],
                "tags": "#MarketingInsights #GuestIntro",
                "img": "/img/guest_intro.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a guest speaker viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The guest speaker should appear modern and abstract, set against a strictly white background.",
                "cta": "Meet Sherif",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-some-to-right"
                }
            },
            {
                "timestamp": "00:15:000",
                "audio": "/audio/03-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Lead Generation Hesitation",
                "text": ["Today, we're diving into a topic that many businesses grapple with: Why Businesses Hesitate to Implement Lead Generation. Sherif, you've worked with numerous businesses over the years. Why do you think there's so much hesitation around lead generation?"],
                "tags": "#LeadGeneration #Marketing",
                "img": "/img/lead_generation.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team discussing strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Discover More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left-to-right",
                }
            },
            {
                "timestamp": "00:30:000",
                "audio": "/audio/04-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Misconceptions",
                "text": ["That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective. While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming."],
                "tags": "#LeadGeneration #Misconceptions",
                "img": "/img/misconceptions.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "00:50:000",
                "audio": "/audio/05-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Immediate Results",
                "text": ["That makes sense. So, what would you say to businesses that believe lead generation doesn't yield immediate results?"],
                "tags": "#LeadGeneration #Results",
                "img": "/img/immediate_results.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a graph showing rapid results viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
                "cta": "See Results",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:00:000",
                "audio": "/audio/06-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly. The key is to set realistic expectations and have a solid plan."],
                "tags": "#LeadGeneration #Strategies",
                "img": "/img/effective_strategies.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a strategic plan viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The strategic plan should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:20:000",
                "audio": "/audio/07-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Audience Understanding",
                "text": ["Absolutely. Now, let's talk about some of the challenges businesses face when implementing lead generation. What are some of the common obstacles?"],
                "tags": "#LeadGeneration #Challenges",
                "img": "/img/audience_understanding.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Face Challenges",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:30:000",
                "audio": "/audio/08-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Common Obstacles",
                "text": ["One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected, leading to frustration and wasted resources."],
                "tags": "#LeadGeneration #Obstacles",
                "img": "/img/common_obstacles.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a frustrated businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Overcome Obstacles",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "01:50:000",
                "audio": "/audio/09-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Fear of the Unknown",
                "text": ["And what about the fear of the unknown? I've heard that many businesses simply don't know where to start."],
                "tags": "#LeadGeneration #Fear",
                "img": "/img/fear_of_unknown.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a confused business team viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Start Here",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:00:000",
                "audio": "/audio/10-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Guidance",
                "text": ["That's a significant barrier, Mike. Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support to get started on the right foot."],
                "tags": "#LeadGeneration #ExpertGuidance",
                "img": "/img/expert_guidance.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business consultation viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business consultation should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Support",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:20:000",
                "audio": "/audio/11-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Practical Solutions",
                "text": ["Great points, Sherif. So, what are some practical solutions to overcome these barriers and successfully implement lead generation?"],
                "tags": "#LeadGeneration #Solutions",
                "img": "/img/practical_solutions.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business meeting discussing solutions viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Find Solutions",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:30:000",
                "audio": "/audio/12-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Education and Training",
                "text": ["First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices. There are numerous online courses, webinars, and resources available that can help demystify the process."],
                "tags": "#LeadGeneration #Education",
                "img": "/img/education_training.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business training session viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The training session should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "02:50:000",
                "audio": "/audio/13-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Start Small",
                "text": ["Starting small and scaling up can also be effective, right?"],
                "tags": "#LeadGeneration #StartSmall",
                "img": "/img/start_small.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a small business team working on a project viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The small business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Begin Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:00:000",
                "audio": "/audio/14-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["Exactly, Mike. Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."],
                "tags": "#LeadGeneration #EffectiveStrategies",
                "img": "/img/effective_strategies_2.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team planning strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:20:000",
                "audio": "/audio/15-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Leverage Technology",
                "text": ["And technology plays a big role too, I assume?"],
                "tags": "#LeadGeneration #Technology",
                "img": "/img/technology.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of technology tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The technology tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Use Tech",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:30:000",
                "audio": "/audio/16-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Tech for Optimization",
                "text": ["Absolutely. There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."],
                "tags": "#LeadGeneration #Optimization",
                "img": "/img/tech_optimization.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of CRM and email marketing tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Optimize Now",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "03:50:000",
                "audio": "/audio/17-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Partnership",
                "text": ["What about partnering with experts? How can that help?"],
                "tags": "#LeadGeneration #ExpertPartnership",
                "img": "/img/expert_partnership.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business partnership viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business partnership should appear modern and abstract, set against a strictly white background.",
                "cta": "Partner Up",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:00:000",
                "audio": "/audio/18-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Get Expert Help",
                "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference. They bring the expertise and experience needed to navigate the complexities of lead generation and ensure successful implementation."],
                "tags": "#LeadGeneration #ExpertHelp",
                "img": "/img/get_expert_help.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business expert providing guidance viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business expert should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Help",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:20:000",
                "audio": "/audio/19-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Conclusion",
                "text": ["Thanks for those insights, Sherif. In conclusion, while businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. By educating themselves, starting small, leveraging technology, and seeking expert guidance, businesses can overcome these barriers and unlock the full potential of lead generation."],
                "tags": "#LeadGeneration #Conclusion",
                "img": "/img/conclusion.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business team celebrating success viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Started",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:40:000",
                "audio": "/audio/20-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Thank You",
                "text": ["Exactly, Mike. Lead generation can truly transform a business when done right."],
                "tags": "#LeadGeneration #ThankYou",
                "img": "/img/thank_you.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a thank you message viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The thank you message should appear modern and abstract, set against a strictly white background.",
                "cta": "Thank You",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            },
            {
                "timestamp": "04:50:000",
                "audio": "/audio/21-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Final Words",
                "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. And don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."],
                "tags": "#LeadGeneration #FinalWords",
                "img": "/img/final_words.webp",
                "img": "/img/episode_start.webp",
                "filter": "/img/filter-comic-3.jpg",
                "backdrop": "/img/backdrop-studio-inside-03.png",
                "frontdrop": "/img/frontdrop-studio-inside_03.png",
                "imgDescription": "A 3D sketch style drawing of a business handshake viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business handshake should appear modern and abstract, set against a strictly white background.",
                "cta": "Subscribe",
                "animation": {
                    "isLooped": false,
                    "isExitAnimated": true,
                    "isEntryAnimated": true,
                    "template": "slide-from-left",
                }
            }
        ]





    }
]
const campaigns_3 = [
    {
        "title": "how to inhace your business",
        "description": "this is a description",
        "img": "/img/leads.png",
        "parentUrl": "/blog/leads",
        "tags": ["#leads", "#business"],
        "category": "business",
        "date": "2022-01-01",
        "author": "john doe",
        "type": "blog",
        "audio": {
            "url": "/audio/01-pod-cast.aac",
            "duration": "04:50:000",
        },
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "ads": [
            {
                "timestamp": "00:00:000",
                "audio": "/audio/01-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Podcast Starts",
                "text": ["Welcome back to another episode of Marketing Insights, the podcast where we explore the latest trends, challenges, and opportunities in the world of marketing and business growth. I'm your host, Mike Sanshase, and today, we have a special guest with us, Sherif Butt, who has over 10 years of experience in the field. Welcome, Sherif!"],
                "tags": "#MarketingInsights #PodcastIntro",
                "img": "/img/episode_start.webp",
                "imgDescription": "A 3D sketch style drawing of a podcast studio viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The podcast studio should appear modern and abstract, set against a strictly white background.",
                "cta": "Stay Tuned",
                "animation": {
                    "image": "fade-in",
                    "cta": "hidden-element",
                    "tags": "hidden-element",
                    "text": "hidden-element",
                    "headline": "fade-in",
                    "logoIcon": "hidden-element",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "00:10:000",
                "audio": "/audio/02-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Guest Introduction",
                "text": ["Thanks, Mike! It's great to be here."],
                "tags": "#MarketingInsights #GuestIntro",
                "img": "/img/guest_intro.webp",
                "imgDescription": "A 3D sketch style drawing of a guest speaker viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The guest speaker should appear modern and abstract, set against a strictly white background.",
                "cta": "Meet Sherif",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-right",
                    "text": "slide-from-right",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "00:15:000",
                "audio": "/audio/03-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Lead Generation Hesitation",
                "text": ["Today, we're diving into a topic that many businesses grapple with: Why Businesses Hesitate to Implement Lead Generation. Sherif, you've worked with numerous businesses over the years. Why do you think there's so much hesitation around lead generation?"],
                "tags": "#LeadGeneration #Marketing",
                "img": "/img/lead_generation.webp",
                "imgDescription": "A 3D sketch style drawing of a business team discussing strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Discover More",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "slide-from-left",
                    "logoIcon": "hidden-element",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "00:30:000",
                "audio": "/audio/04-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Misconceptions",
                "text": ["That's a great question, Mike. There are several reasons businesses hesitate. One of the biggest misconceptions is that lead generation is overly complex and requires a significant amount of time and resources to be effective. While it's true that it requires a well-thought-out strategy and consistent effort, it doesn't have to be overwhelming."],
                "tags": "#LeadGeneration #Misconceptions",
                "img": "/img/misconceptions.webp",
                "imgDescription": "A 3D sketch style drawing of a confused businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "image": "slide-from-right",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "slide-from-left",
                    "headline": "slide-from-left",
                    "logoIcon": "hidden-element",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "00:50:000",
                "audio": "/audio/05-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Immediate Results",
                "text": ["That makes sense. So, what would you say to businesses that believe lead generation doesn't yield immediate results?"],
                "tags": "#LeadGeneration #Results",
                "img": "/img/immediate_results.webp",
                "imgDescription": "A 3D sketch style drawing of a graph showing rapid results viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The graph should appear modern and abstract, set against a strictly white background.",
                "cta": "See Results",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "01:00:000",
                "audio": "/audio/06-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["It's important to understand that the effectiveness of lead generation can vary based on the approach and the industry. Some strategies might take time to build momentum, but others, like targeted online ads or email campaigns, can start showing results relatively quickly. The key is to set realistic expectations and have a solid plan."],
                "tags": "#LeadGeneration #Strategies",
                "img": "/img/effective_strategies.webp",
                "imgDescription": "A 3D sketch style drawing of a strategic plan viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The strategic plan should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "01:20:000",
                "audio": "/audio/07-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Audience Understanding",
                "text": ["Absolutely. Now, let's talk about some of the challenges businesses face when implementing lead generation. What are some of the common obstacles?"],
                "tags": "#LeadGeneration #Challenges",
                "img": "/img/audience_understanding.webp",
                "imgDescription": "A 3D sketch style drawing of a business meeting viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Face Challenges",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "01:30:000",
                "audio": "/audio/08-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Common Obstacles",
                "text": ["One major challenge is the lack of understanding of the target audience. Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected, leading to frustration and wasted resources."],
                "tags": "#LeadGeneration #Obstacles",
                "img": "/img/common_obstacles.webp",
                "imgDescription": "A 3D sketch style drawing of a frustrated businessperson viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The businessperson should appear modern and abstract, set against a strictly white background.",
                "cta": "Overcome Obstacles",
                "animation": {
                    "image": "slide-from-right",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "slide-from-left",
                    "headline": "slide-from-left",
                    "logoIcon": "hidden-element",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "01:50:000",
                "audio": "/audio/09-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Fear of the Unknown",
                "text": ["And what about the fear of the unknown? I've heard that many businesses simply don't know where to start."],
                "tags": "#LeadGeneration #Fear",
                "img": "/img/fear_of_unknown.webp",
                "imgDescription": "A 3D sketch style drawing of a confused business team viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Start Here",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "02:00:000",
                "audio": "/audio/10-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Guidance",
                "text": ["That's a significant barrier, Mike. Many businesses lack the in-house expertise and are unsure about outsourcing this critical function. However, partnering with experienced lead generation professionals or agencies can provide the necessary guidance and support to get started on the right foot."],
                "tags": "#LeadGeneration #ExpertGuidance",
                "img": "/img/expert_guidance.webp",
                "imgDescription": "A 3D sketch style drawing of a business consultation viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business consultation should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Support",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "02:20:000",
                "audio": "/audio/11-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Practical Solutions",
                "text": ["Great points, Sherif. So, what are some practical solutions to overcome these barriers and successfully implement lead generation?"],
                "tags": "#LeadGeneration #Solutions",
                "img": "/img/practical_solutions.webp",
                "imgDescription": "A 3D sketch style drawing of a business meeting discussing solutions viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business meeting should appear modern and abstract, set against a strictly white background.",
                "cta": "Find Solutions",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "02:30:000",
                "audio": "/audio/12-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Education and Training",
                "text": ["First, education and training are key. Businesses should invest time in learning about lead generation strategies, tools, and best practices. There are numerous online courses, webinars, and resources available that can help demystify the process."],
                "tags": "#LeadGeneration #Education",
                "img": "/img/education_training.webp",
                "imgDescription": "A 3D sketch style drawing of a business training session viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The training session should appear modern and abstract, set against a strictly white background.",
                "cta": "Learn More",
                "animation": {
                    "image": "slide-from-right",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "slide-from-left",
                    "headline": "slide-from-left",
                    "logoIcon": "hidden-element",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "02:50:000",
                "audio": "/audio/13-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Start Small",
                "text": ["Starting small and scaling up can also be effective, right?"],
                "tags": "#LeadGeneration #StartSmall",
                "img": "/img/start_small.webp",
                "imgDescription": "A 3D sketch style drawing of a small business team working on a project viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The small business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Begin Now",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "03:00:000",
                "audio": "/audio/14-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Effective Strategies",
                "text": ["Exactly, Mike. Begin with a simple lead generation campaign to test the waters. Use the insights gained from this initial effort to refine your strategy and gradually expand your efforts."],
                "tags": "#LeadGeneration #EffectiveStrategies",
                "img": "/img/effective_strategies_2.webp",
                "imgDescription": "A 3D sketch style drawing of a business team planning strategies viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Plan Now",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "03:20:000",
                "audio": "/audio/15-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Leverage Technology",
                "text": ["And technology plays a big role too, I assume?"],
                "tags": "#LeadGeneration #Technology",
                "img": "/img/technology.webp",
                "imgDescription": "A 3D sketch style drawing of technology tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The technology tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Use Tech",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "03:30:000",
                "audio": "/audio/16-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Tech for Optimization",
                "text": ["Absolutely. There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate and optimize lead generation efforts."],
                "tags": "#LeadGeneration #Optimization",
                "img": "/img/tech_optimization.webp",
                "imgDescription": "A 3D sketch style drawing of CRM and email marketing tools viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The tools should appear modern and abstract, set against a strictly white background.",
                "cta": "Optimize Now",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "03:50:000",
                "audio": "/audio/17-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Expert Partnership",
                "text": ["What about partnering with experts? How can that help?"],
                "tags": "#LeadGeneration #ExpertPartnership",
                "img": "/img/expert_partnership.webp",
                "imgDescription": "A 3D sketch style drawing of a business partnership viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business partnership should appear modern and abstract, set against a strictly white background.",
                "cta": "Partner Up",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "04:00:000",
                "audio": "/audio/18-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Get Expert Help",
                "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference. They bring the expertise and experience needed to navigate the complexities of lead generation and ensure successful implementation."],
                "tags": "#LeadGeneration #ExpertHelp",
                "img": "/img/get_expert_help.webp",
                "imgDescription": "A 3D sketch style drawing of a business expert providing guidance viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business expert should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Help",
                "animation": {
                    "image": "slide-from-left",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "04:20:000",
                "audio": "/audio/19-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Conclusion",
                "text": ["Thanks for those insights, Sherif. In conclusion, while businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. By educating themselves, starting small, leveraging technology, and seeking expert guidance, businesses can overcome these barriers and unlock the full potential of lead generation."],
                "tags": "#LeadGeneration #Conclusion",
                "img": "/img/conclusion.webp",
                "imgDescription": "A 3D sketch style drawing of a business team celebrating success viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business team should appear modern and abstract, set against a strictly white background.",
                "cta": "Get Started",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            },
            {
                "timestamp": "04:40:000",
                "audio": "/audio/20-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Thank You",
                "text": ["Exactly, Mike. Lead generation can truly transform a business when done right."],
                "tags": "#LeadGeneration #ThankYou",
                "img": "/img/thank_you.webp",
                "imgDescription": "A 3D sketch style drawing of a thank you message viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The thank you message should appear modern and abstract, set against a strictly white background.",
                "cta": "Thank You",
                "animation": {
                    "image": "slide-from-right",
                    "cta": "slide-from-bottom",
                    "tags": "fade-in",
                    "text": "fade-in",
                    "headline": "slide-from-left",
                    "logoIcon": "slide-from-right",
                    "pathTwo": "hidden-element",
                    "pathOne": "hidden-element"
                }
            },
            {
                "timestamp": "04:50:000",
                "audio": "/audio/21-pod-cast.aac",
                "logo": {
                    "icon": "fa-brands fa-octopus-deploy",
                    "firstWord": "octopus",
                    "secondWord": "energy"
                },
                "headline": "Final Words",
                "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. And don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."],
                "tags": "#LeadGeneration #FinalWords",
                "img": "/img/final_words.webp",
                "imgDescription": "A 3D sketch style drawing of a business handshake viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The business handshake should appear modern and abstract, set against a strictly white background.",
                "cta": "Subscribe",
                "animation": {
                    "image": "fade-in",
                    "cta": "slide-from-bottom",
                    "tags": "slide-from-left",
                    "text": "slide-from-left",
                    "headline": "fade-in",
                    "logoIcon": "slide-from-left",
                    "pathTwo": "slide-from-bottom",
                    "pathOne": "slide-from-bottom"
                }
            }
        ]





    }
]

const ads3 = [
    {
        "timestamp": 0,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Podcast Starts Now",
        "text": ["Welcome back to Marketing Insights, the podcast exploring trends, challenges, and opportunities in marketing and business growth. I'm Mike Sanshase, your host."],
        "tags": "#MarketingInsights #PodcastIntro",
        "img": "/img/podcast_start.webp",
        "imgDescription": "A 3D sketch style drawing of a microphone viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The microphone should appear modern and abstract, set against a strictly white background.",
        "cta": "Stay Tuned",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 13000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Special Guest",
        "text": ["Today, we have Sherif Butt, a seasoned expert with over 10 years of experience, joining us to discuss why businesses hesitate to implement lead generation."],
        "tags": "#SpecialGuest #LeadGeneration",
        "img": "/img/sherif.png",
        "imgDescription": "A 3D sketch style drawing of Sherif Butt viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). Sherif should appear professional and approachable, set against a strictly white background.",
        "cta": "Listen Now",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 25000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Hesitation Reasons",
        "text": ["Many businesses believe lead generation is overly complex and time-consuming. However, with the right strategy and effort, it can be manageable."],
        "tags": "#LeadGeneration #BusinessHesitation",
        "img": "/img/lead_generation_intro.webp",
        "imgDescription": "A 3D sketch style drawing of a person at a desk surrounded by papers and a laptop, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear busy but focused, set against a strictly white background.",
        "cta": "Learn More",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 37000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Overcoming Misconceptions",
        "text": ["It's important to understand that lead generation can vary based on the approach and the industry. Some strategies show results quickly, like targeted ads or email campaigns."],
        "tags": "#LeadGeneration #Misconceptions",
        "img": "/img/overcoming_misconceptions.webp",
        "imgDescription": "A 3D sketch style drawing of a person looking at a computer screen with charts and graphs, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear analytical and data-driven, set against a strictly white background.",
        "cta": "Understand More",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-top",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 49000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Challenges Faced",
        "text": ["Effective lead generation relies on knowing who your potential customers are, what they need, and how to reach them. Without this understanding, efforts can be misdirected."],
        "tags": "#LeadGeneration #Challenges",
        "img": "/img/challenges_faced.webp",
        "imgDescription": "A 3D sketch style drawing of a person holding a target and arrow, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear focused and goal-oriented, set against a strictly white background.",
        "cta": "Learn How",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-right",
            "text": "slide-from-right",
            "headline": "slide-from-right",
            "logoIcon": "slide-from-right",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 61000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Fear of Unknown",
        "text": ["Many businesses are unsure of where to start with lead generation. Partnering with experienced professionals can provide the necessary guidance to get started on the right foot."],
        "tags": "#LeadGeneration #FearOfUnknown",
        "img": "/img/fear_of_unknown.webp",
        "imgDescription": "A 3D sketch style drawing of two people shaking hands, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear collaborative and supportive, set against a strictly white background.",
        "cta": "Get Started",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-left",
            "tags": "slide-from-right",
            "text": "slide-from-right",
            "headline": "slide-from-right",
            "logoIcon": "slide-from-right",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 73000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Practical Solutions",
        "text": ["Education and training are key. Invest time in learning about lead generation strategies, tools, and best practices to demystify the process."],
        "tags": "#LeadGeneration #PracticalSolutions",
        "img": "/img/practical_solutions.webp",
        "imgDescription": "A 3D sketch style drawing of a person reading a book with charts in the background, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear educational and insightful, set against a strictly white background.",
        "cta": "Invest Time",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 85000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Start Small",
        "text": ["Begin with a simple lead generation campaign to test the waters. Use the insights gained to refine your strategy and gradually expand your efforts."],
        "tags": "#LeadGeneration #StartSmall",
        "img": "/img/start_small.webp",
        "imgDescription": "A 3D sketch style drawing of a person holding a small plant, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear nurturing and growth-focused, set against a strictly white background.",
        "cta": "Test Waters",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-left",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 97000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Leveraging Technology",
        "text": ["There are many lead generation tools and platforms designed to streamline the process. From CRM systems to email marketing software, these tools can help automate efforts."],
        "tags": "#LeadGeneration #Technology",
        "img": "/img/leveraging_technology.webp",
        "imgDescription": "A 3D sketch style drawing of a person using a laptop with various tech icons around, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear tech-savvy and efficient, set against a strictly white background.",
        "cta": "Use Tools",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 109000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Partner with Experts",
        "text": ["Partnering with experts, whether it's a consultancy, a marketing agency, or hiring an experienced professional in-house, can make a significant difference."],
        "tags": "#LeadGeneration #ExpertPartner",
        "img": "/img/partner_experts.webp",
        "imgDescription": "A 3D sketch style drawing of a group of people in a meeting, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear collaborative and professional, set against a strictly white background.",
        "cta": "Get Help",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-right",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 121000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Conclusion",
        "text": ["While businesses may hesitate to implement lead generation due to misconceptions and challenges, the benefits far outweigh the initial hurdles. Educate, start small, leverage technology, and seek expert guidance."],
        "tags": "#LeadGeneration #Conclusion",
        "img": "/img/conclusion.webp",
        "imgDescription": "A 3D sketch style drawing of a person standing at a crossroads, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear thoughtful and decisive, set against a strictly white background.",
        "cta": "Act Now",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-top",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    },
    {
        "timestamp": 133000,
        "audio": "/audio/01-pod-cast.aac",
        "logo": {
            "icon": "fa-brands fa-octopus-deploy",
            "firstWord": "octopus",
            "secondWord": "energy"
        },
        "headline": "Outro",
        "text": ["Thank you for joining us today, Sherif. For our listeners who want to learn more about lead generation, be sure to visit Sherif's website at test.loyalleads.co.uk. Don't forget to subscribe to our podcast for more insights and tips on marketing and business growth. Until next time, I'm Mike Sanshase, and this has been Marketing Insights."],
        "tags": "#PodcastOutro #MarketingInsights",
        "img": "/img/outro.webp",
        "imgDescription": "A 3D sketch style drawing of a podcast studio setup, viewed from a slightly elevated angle, showing the top and one side. The design is minimalistic, featuring clean lines with two accents: black and #ffd84d (bright yellow). The scene should appear professional and engaging, set against a strictly white background.",
        "cta": "Subscribe Now",
        "animation": {
            "image": "fade-in",
            "cta": "slide-from-bottom",
            "tags": "slide-from-left",
            "text": "slide-from-left",
            "headline": "slide-from-left",
            "logoIcon": "slide-from-left",
            "pathTwo": "hidden-element",
            "pathOne": "hidden-element"
        }
    }
]


const bannerSettings = [

    {
        "size": "1200x628",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "default",
                "imageOpacity": 1,
                "svgOneOpacity": 0.53,
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "43%",
                "titleSize": "2.9rem",
                "bodySize": "1.25rem",
                "switchPosition": false,
                "imageZIndex": 16,
                "textLength": 0,
                "zIndex": {
                    "image": 24
                }
            }
        }
    },
    {
        "size": "480x120",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "circle",
                "imageOpacity": "1",
                "svgOneOpacity": "1",
                "svgTwoOpacity": "1",
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "27%",
                "titleSize": "1.2rem",
                "bodySize": "1rem",
                "switchPosition": false,
                "imageZIndex": 29,
                "textLength": 0
            }
        }
    },
    {
        "size": "300x250",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-col",
                "ctaStyle": "circle",
                "imageOpacity": 1,
                "svgOneOpacity": "1",
                "svgTwoOpacity": "1",
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "53%",
                "titleSize": "1.4rem",
                "bodySize": "1rem",
                "switchPosition": true,
                "imageZIndex": 9,
                "textLength": 2
            }
        }
    },
    {
        "size": "160x600",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "circle",
                "imageOpacity": 1,
                "svgOneOpacity": "1",
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "100%",
                "titleSize": "1.6rem",
                "bodySize": "0.9rem",
                "switchPosition": false,
                "imageZIndex": 9,
                "textLength": 0
            }
        }
    },
    {
        "size": "300x250-text",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "circle",
                "imageOpacity": 1,
                "svgOneOpacity": "1",
                "svgTwoOpacity": "1",
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "0%",
                "titleSize": "1.3rem",
                "bodySize": "0.9rem",
                "switchPosition": false,
                "imageZIndex": 30,
                "textLength": 0
            }
        }
    },
    {
        "size": "728x90",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "circle",
                "imageOpacity": 1,
                "svgOneOpacity": "1",
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "15%",
                "titleSize": "1.2rem",
                "bodySize": "1rem",
                "switchPosition": false,
                "imageZIndex": 30,
                "textLength": 0
            }
        }
    },
    {
        "size": "1200x628-2",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "default",
                "imageOpacity": 1,
                "svgOneOpacity": 0.53,
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "50%",
                "titleSize": "2rem",
                "bodySize": "1.25rem",
                "switchPosition": false,
                "imageZIndex": 30,
                "textLength": 0,
                "zIndex": {
                    "image": 20
                }
            }
        }
    },
    {
        "size": "1080x1080",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "default",
                "imageOpacity": 1,
                "svgOneOpacity": 0.53,
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "50%",
                "titleSize": "4.9rem",
                "bodySize": "1.25rem",
                "switchPosition": false,
                "imageZIndex": 30,
                "textLength": 0,
                "positions": {
                    "path": "translate(-10, -30)",
                    "svg": {
                        "top": "-5px",
                        "left": ""
                    },
                    "ad": {
                        "top": "",
                        "left": "20px"
                    },
                    "text": {
                        "top": "-45px",
                        "left": ""
                    },
                    "textcontainer": {
                        "top": "875px",
                        "left": "100px"
                    },
                    "headline": {
                        "top": "-75px",
                        "left": "175px"
                    },
                    "cta": {
                        "top": "105px",
                        "left": "300px"
                    },
                    "image": {
                        "top": "150px",
                        "left": "240px"
                    }
                }
            }
        }
    },
    {
        "size": "720x1280",
        "settings": {
            "elements": {
                "logoIcon": true,
                "image": true,
                "headline": true,
                "text": false,
                "tags": true,
                "cta": true,
                "svgWave": true,
                "textContainer": true,
                "backdrop": false,
                "filter": false,
                "backbanner": false,
                "frontdrop": false,
            },
            "layout": {
                "logoLayout": "flex-row",
                "ctaStyle": "default",
                "imageOpacity": 1,
                "svgOneOpacity": 0.53,
                "svgTwoOpacity": 1,
                "logoRotation": 0,
                "backgroundImage": false,
                "imageSize": "65%",
                "titleSize": "4.9rem",
                "bodySize": "1.25rem",
                "switchPosition": false,
                "imageZIndex": 30,
                "textLength": 0,
                "positions": {
                    "path": "translate(-10, -30)",
                    "svg": {
                        "top": "-5px",
                        "left": ""
                    },
                    "ad": {
                        "top": "",
                        "left": "20px"
                    }
                }
            }
        }
    }

]

const filters = [
    "/img/filter-comic.png",
    "/img/filter-34294033_dotted_background.png",
    "/img/filter-10308569_18448119.png",
    "/img/filter-32499109_7963948.png",
    "/img/filter-flat-design-black-white-halftone.png",

]